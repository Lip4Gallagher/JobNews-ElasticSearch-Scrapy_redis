...
def result(request):
    ...
    try:
        ...
        if keyword and keyword != '':
            if zone == '' or zone == '不限':
                ...
                must = [{"match": {"job": keyword}}, {"match": {"zone": zone}},
                        {"range": {"educated": {"lte": educated}}}]
        else:
    ...
    if educated and zone:
        page_urls = [
            [i, '?keyword=' + str(keyword) + '&zone=' + str(zone) + '&education=' + str(educated) + '&page=' + str(i)]
            for i in pages]

    elif (not educated) and (not zone):
        page_urls = [[i, '?keyword=' + str(keyword) + '&page=' + str(i)] for i in pages]

    experience = {'-2': '经验不限', '-1': '在读学生', '0': '应届毕业生', '1': '一年', '2': '两年', '3': '三年', '4': '四年', '5': '五年',
                  '6': '六年', '7': '七年', '8': '八年', '9': '九年', '10': '十年及以上'}
    ...
    return render('result.html', locals())
...
def show(request):
    ...
    return render('show.html', locals())
